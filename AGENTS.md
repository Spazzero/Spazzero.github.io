...
- **Reading page**: Books live in `src/data/reading.ts`. `getBooks` in `src/lib/reading.ts` sorts by `finished`, so array order does not matter. `note` is optional; a book with one renders as a native `<details>` row, which keeps the page free of client JavaScript
...
- **Hierarchy from data**: Where a list needs varying visual weight (e.g. `tierFor` in `src/components/Resume/Experience.tsx`), derive it from the data — dates — rather than hardcoding per-item styling, so new entries place themselves. `tierFor` deliberately does not read role titles: it used to step any role matching `/intern/i` down to the tail, which inverts the hierarchy on a resume whose substantive roles are internships — every real position renders small and the one job that happens not to say "intern" is promoted to lead
