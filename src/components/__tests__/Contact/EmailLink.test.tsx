...
describe('EmailLink', () => {
  ...
  it('never blanks or snaps back to the address mid-animation', () => {
    ...
    for (let elapsed = 0; elapsed < 120_000; elapsed += 50) {
      ...
      // already several characters long. Looping re-types the address one
      // character at a time, which is legitimate and must not be flagged.
      //
      ...
      const isTypingThroughAddress =
        localPart.startsWith(previous) && shown.length === previous.length + 1;

      if (
        previous.length > 1 &&
        previous !== localPart &&
        !isTypingThroughAddress
      ) {
