...
/**
 * `twitter:site` and `twitter:creator` name an X account. They are optional in
 * the card spec, and a site with no account omits them rather than shipping an
 * empty tag, so requiring them outright fails every page of a perfectly valid
 * export.
 *
 * The property actually worth protecting is consistency: a route-level
 * `twitter` object replaces the inherited one wholesale, so a page can silently
 * lose tags the rest of the site has. These are therefore required on every
 * indexable page only once any page declares them.
 */
const CONDITIONAL_SOCIAL_META = [
  ['name', 'twitter:site'],
  ['name', 'twitter:creator'],
];

const declaredSocialMeta = new Set(
  CONDITIONAL_SOCIAL_META.filter(([key, name]) =>
    records.some(
      (record) =>
        record.isIndexable && metaValues(record.html, key, name).length > 0,
    ),
  ).map(([, name]) => name),
);
...
for (const record of records) {
  ...
  if (isIndexable) {
    const applicable = [
      ...
      ...CONDITIONAL_SOCIAL_META.filter(([, name]) =>
        declaredSocialMeta.has(name),
      ),
    ];

    for (const [key, name] of applicable) {
